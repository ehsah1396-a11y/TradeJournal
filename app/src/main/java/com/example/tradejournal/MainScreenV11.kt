...
fun App(vm: TradeViewModel) {
    ...
    var onboardDone by remember { mutableStateOf(Onboarding.done(ctx)) }
    ...
    if (!onboardDone) {
        OnboardingScreen(vm.mascot) { Onboarding.setDone(ctx); onboardDone = true }
        return
    }
                ...
                bottomBar = {
                    if (screen != 2) Column {
                        ...
                        NavigationBar(containerColor = ColCard) {
                            ...
                            NavigationBarItem(screen == 3 || screen == 5, { screen = 3 },
