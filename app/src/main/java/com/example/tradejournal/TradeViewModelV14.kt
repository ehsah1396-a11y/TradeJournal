...
class TradeViewModel(app: Application) : AndroidViewModel(app) {
    ...
    var mode by mutableIntStateOf(sp.getInt("mode", 2))       // 0 خودکار، 1 شب، 2 روز (پیش‌فرض: روز)
    ...
    /** پرو همیشگی را فعال می‌کند (بعد از خرید یک‌بارهٔ pro_lifetime) */
    fun grantProForever() {
        proUntil = Long.MAX_VALUE
        sp.edit().putLong("proUntil", Long.MAX_VALUE).apply()
    }

    /** بعد از خرید یا تمدید اشتراک ماهانه‌ی pro_monthly صدا زده می‌شود */
    fun grantProMonthly() {
        if (proUntil == Long.MAX_VALUE) return // همیشگی از اشتراک ماهانه بالاتره، دست نخوره
        val base = maxOf(proUntil, System.currentTimeMillis())
        proUntil = base + 30L * 24 * 60 * 60 * 1000
        sp.edit().putLong("proUntil", proUntil).apply()
    }
